...
app.post("/api/logs", (req, res) => {
  ...
  if (Array.isArray(logs)) {
    logs.forEach((log) => {
      // Store frontend logs in the same format
      // In production, consider persisting these to a database
...
app.get("/api/curriculum", async (req, res) => {
  try {
    ...
    // Format to match old curriculum format:
    // [ { id: 'math', label: 'Mathematics', chapters: [ { id: 'algebra', label: 'Algebra', topics: [ 'Linear equations', ... ] } ] } ]
...
app.get("/api/content/:sid/:cid/:topic", async (req, res) => {
  ...
  try {
    ...
    // Rebuild the "qs" array matching the old format
    const qs = [];
...
app.post("/api/grade", async (req, res) => {
  ...
  try {
    ...
    // Reconstruct normalizeQ behavior dynamically
    const question = {
    ...
    // If correctAnswer is an array (multiple correct variants), check against all of them
    let isCorrect = false;
    ...
    if (Array.isArray(correctAnswer)) {
      ...
      isCorrect = correctAnswer.some((variant) => {
        ...
        if (uAns.length > 3 && cAns.length > 0) {
          ...
          if (
            keywords.length > 0 &&
            (keywordRatio >= 0.5 || overlapRatio >= 0.45)
          ) {
            return true;
          }
      ...
      // Allow shorter answers or abbreviations if meaning is preserved
      ...
      if (!isCorrect && uAns.length > 3 && cAns.length > 0) {
        ...
        // Accept answers that contain key phrases from the expected answer
        const commonPhrases = [
          cAns,
          ...correctTokens
            .slice(0, 3)
        ...
        if (
          !isCorrect &&
          commonPhrases.some((phrase) => phrase && uAns.includes(phrase))
        ) {
          isCorrect = true;
        }
      }
...
app.post("/api/analytics/events", async (req, res) => {
  ...
  try {
    // Resolve topic_ids for each event using content_view
    const rowsToInsert = [];
...
app.use((err, req, res, next) => {
  try {
    logger.error("EXPRESS_ERROR", err, {
      endpoint: req.path,
      method: req.method,
    });
  } catch (loggingError) {
...
process.on("unhandledRejection", (reason, promise) => {
  try {
    logger.error("UNHANDLED_REJECTION", new Error(String(reason)));
  } catch (err) {
