  ...
  *
  * Variation is controlled explicitly through variantIndex.
  *
...
export class GeographyMutator {
  ...
  mutate(qObj, modalityIndex = null) {
     ...
     * IMPORTANT:
     *
     * There is NO RANDOM DEFAULT anymore.
     *
     * If modalityIndex is supplied, use it.
     * Otherwise use the question's variantIndex.
     * If neither exists, default to 0.
     */

    const mode = this.normalizeModality(
    ...
    const rotate = (arr, index = 0) => {
      ...
      return [
        ...arr.slice(normalizedIndex),
        ...arr.slice(0, normalizedIndex)
      ];
    };
    ...
    const uniqueOptions = (correct, distractors) => {
      ...
      /*
       * Correct answer remains part of the option set.
       * Ordering is deterministic.
       */
      return rotate(unique.slice(0, 4), variantIndex);
    };
    ...
    if (
      ...
      /*
       * Instead of randomly selecting a map topic,
       * use the first matching topic according to priority.
       *
       * This makes detection stable.
       */

      let selectedTopic = "scale";

      if (lower.includes("bearing")) {
        selectedTopic = "bearing";
      } else if (
        lower.includes("contour") ||
        lower.includes("gradient")
      ) {
      ...
      if (selectedTopic === "scale") {
        ...
        const selected =
          cases[variantIndex % cases.length];

        const scale = selected.scale;
        const mapDistance = selected.mapDistance;
        ...
        if (mode === 0) {
          ...
          return makeResult({
            ...
            why:
              `Ground distance = ${mapDistance} × ` +
              `${scale.toLocaleString()} cm = ` +
        ...
        if (mode === 1) {
          ...
          return makeResult({
            q:
              `A road is ${mapDistance} cm long on a ` +
              `1:${scale.toLocaleString()} map. ` +
            ...
            steps: [
              "Step 1: Identify the map distance",
              "Step 2: Multiply by the scale denominator",
        ...
        if (mode === 2) {
          ...
          return makeResult({
            ...
            ans:
              `Incorrect. The correct ground distance is ${answer}.`,
            ...
            sol:
              `Incorrect. Correct distance = ${answer}.`,
        ...
        return makeResult({
          q:
            `State the formula for finding ground distance from map ` +
            `distance and calculate the ground distance represented by ` +
            `${mapDistance} cm on a 1:${scale.toLocaleString()} map.`,
      ...
      if (selectedTopic === "bearing") {
        ...
        const bearing =
          bearings[variantIndex % bearings.length];

        const reverseBearing =
          (bearing + 180) % 360 || 360;

        const answer =
          `${String(reverseBearing).padStart(3, "0")}°`;
        ...
        if (mode === 0 || mode === 1) {
          ...
          return makeResult({
            q:
              `A settlement A has a bearing of ` +
            ...
            steps: [
              ...
              "Step 2: Add or subtract 180° because the direction is reversed",
        ...
        return makeResult({
          ...
          ans:
            ...
            `The correct back bearing is ${answer}.`,
      ...
      if (selectedTopic === "gradient") {
        ...
        const verticalRise =
          selected.verticalInterval *
          selected.contourDifference;
        ...
        const ratio =
          Math.round(gradientRatio);

        return makeResult({
          q:
            `Two points on a topographical map have a vertical ` +
            `difference of ${verticalRise} m and a horizontal ` +
            `ground distance of ${horizontalDistance} m. ` +
            `Calculate the gradient as a ratio of vertical interval ` +
    ...
    if (
      ...
      const selected =
        climateCases[
          variantIndex % climateCases.length
        ];
      ...
      if (mode === 0) {
        ...
        return makeResult({
          q:
            `A weather station records the following conditions: ` +
            `${selected.clue}. Identify the most likely climatic region.`,
      ...
      return makeResult({
        q:
          `A student identifies the climate described as ` +
          `"${selected.clue}" as a different climate zone. ` +
          `Explain why that classification is wrong and identify ` +
          `the correct climate.`,

        ans:
          `The correct classification is ${selected.answer}. ` +
          `${selected.why}`,
    ...
    if (
      ...
      const selected =
        tectonicCases[
          variantIndex % tectonicCases.length
        ];
    ...
    if (
      ...
      const selected =
        riverCases[
          variantIndex % riverCases.length
        ];

      return makeResult({
        q:
          `[Fluvial Geomorphology] A river is experiencing ` +
          `${selected.process}. Which landform is most likely to develop?`,
    ...
    if (
      ...
      const selected =
        weatheringCases[
          variantIndex % weatheringCases.length
        ];
    ...
    if (
      ...
      const selected =
        coastalCases[
          variantIndex % coastalCases.length
        ];
      ...
      const cases = [
        {
          ...
          answer:
            "Rapid natural population increase",
    ...
    if (
      ...
      const selected =
        agricultureCases[
          variantIndex % agricultureCases.length
        ];
    ...
    return {
      ...
      q:
        `[Geographic Reasoning Check] ${stem}`,
