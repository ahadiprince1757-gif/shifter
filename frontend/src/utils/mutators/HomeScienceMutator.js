 ...
 *
 * Same question + same variantIndex = same output.
 *
 * Supported:
 * - Nutrition & deficiency diseases
 * - Food hygiene & safety
 * - Food preservation
 * - Textile fibres & clothing care
 * - First aid & household health
 * - Consumer education & budgeting
 * - Child/family resource management
 ...
 *
 * Design principle:
 * Don't merely change words.
 * Change the ANGLE from which mastery is demonstrated.
 */
...
export class HomeScienceMutator {
  ...
  mutate(qObj, modalityIndex = 0) {
    ...
    /*
     * Modality and variant are deliberately independent.
     *
     * modalityIndex:
     *   0 = open response
     *   1 = MCQ
     ...
     *
     * variantIndex:
     *   Controls deterministic content variation.
     */

    const mode =
      this.normalizeModality(modalityIndex);
    ...
    const rotate = (items, index = 0) => {
      ...
      return [
        ...items.slice(normalized),
        ...items.slice(0, normalized)
      ];
    };

    /*
     * Deterministic option generator.
     *
     * The correct answer is always included.
     * Duplicate answers are removed.
     * Ordering depends only on variantIndex.
     */
    ...
    if (
      ...
      const cases = [
...

        {
          ...
          cause:
            "Vitamin D deficiency affecting calcium absorption and bone mineralization",
...

        {
          ...
          cause:
            "Insufficient iron for normal haemoglobin production",
...

        {
          ...
          cause:
            "Severe deficiency of energy and nutrients",
      ...
      if (mode === 0) {
        ...
        return makeResult({
          ...
          why:
            `${selected.condition} is associated with ${selected.cause}.`,

          sol:
            selected.nutrient,

          steps: [
            "Step 1: Identify the major symptoms.",
      ...
      if (mode === 1) {
        ...
        return makeResult({
          ...
          why:
            `${selected.nutrient} deficiency explains the characteristic symptoms described.`,
      ...
      if (mode === 2) {
        ...
        return makeResult({
          ...
          q:
            `[Nutrition Error Check]\n` +
            `${selected.scenario}\n\n` +
            `A learner claims that the condition is caused by ${wrong}. ` +
            `Is the claim correct? Explain briefly and give the correct nutrient.`,

          ans:
            `Incorrect. The nutrient involved is ${selected.nutrient}.`,

          hint:
            selected.hint,

          why:
            `${selected.condition} is associated with ${selected.cause}, not ${wrong}.`,

          sol:
            `Incorrect. The correct nutrient is ${selected.nutrient}.`,

          steps: [
            "Step 1: Identify the symptoms.",
            "Step 2: Compare them with the proposed deficiency.",
      ...
      return makeResult({
        ...
        ans:
          `${selected.nutrient}. ${selected.remedy}`,

        hint:
          `${selected.hint} Then connect the nutrient to suitable food sources.`,

        why:
          `${selected.condition} results from ${selected.cause}. Correct dietary management should restore the deficient nutrient appropriately.`,
    ...
    if (
      ...
      if (mode === 0) {
        ...
        return makeResult({
          ...
          hint:
            "Focus on how food is handled, stored or protected from contamination.",
      ...
      if (mode === 1) {
        ...
        return makeResult({
          ...
          hint:
            "Think about contamination, microorganisms, storage or handling.",
      ...
      if (mode === 2) {
        ...
        return makeResult({
          ...
          q:
            `[Food Safety Error Detection]\n` +
            `${selected.scenario}\n\n` +
            `A learner says this practice is completely safe. ` +
            `Do you agree? Give the correct principle and explain why.`,

          ans:
            `The correct principle is ${selected.answer}. ${selected.explanation}`,

          hint:
            "Look for the pathway by which food could become unsafe.",

          why:
            selected.explanation,

          sol:
            `${selected.answer}. ${selected.prevention}`,
      ...
      return makeResult({
        ...
        hint:
          "State both the principle and the practical corrective action.",
    ...
    if (
      ...
      const selected =
        preservation[
          variantIndex % preservation.length
        ];
      ...
      if (mode === 3) {
        ...
        return makeResult({
          ...
          hint:
            "Consider how the method controls moisture, microorganisms or spoilage.",
    ...
    if (
      ...
      const textileCases = [

        {
          ...
          principle:
            "Cotton absorbs moisture and allows comfortable wear in warm conditions.",
      ...
      const selected =
        textileCases[
          variantIndex % textileCases.length
        ];

      return makeResult({

        q:
          `[Textile Care Scenario]\n` +
          `${selected.scenario}\n\n` +
        ...
        hint:
          "Consider the properties of the fibre and the effect of heat, moisture and mechanical action.",
    ...
    if (
      ...
      return makeResult({
        ...
        hint:
          "Choose the action that protects the injured person and limits further harm.",
    ...
    if (
      ...
      return makeResult({
        ...
        hint:
          "Think about how households make decisions when resources are limited.",
    ...
    return makeResult({
      ...
      q:
        `[Home Science Application Check]\n${stem}`,
